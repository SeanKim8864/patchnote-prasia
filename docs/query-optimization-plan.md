...
- 현재 1차 보호 케이스:
  ...
  - `클래스체인지 기간 알려줘` (`클래스 체인지` 붙여쓰기 변형)
  - `백색증표 언제 업데이트 됐어` (`백색 증표` 붙여쓰기 변형)
  - `부스팅월드 혜택 알려줘` (`부스팅 월드` 붙여쓰기 변형)
- 검증 기준:
  - 클래스 체인지 이력 질의는 기대 회차 수가 모두 나와야 한다.
  - 각 회차의 `start_at`, `end_at`가 기대값과 일치해야 한다.
  - 상위 결과가 `event_record` 경로를 우선으로 유지해야 한다.
  - 붙여쓰기/띄어쓰기 변형 질의도 현재 정규화 맵 범위 안에서는 동일 개체를 안정적으로 찾을 수 있어야 한다.
